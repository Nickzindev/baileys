...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    if (addressingMode === "lid") {
        ...
        participantPn = stanza.attrs.participant 
        participant = stanza.attrs.participant_lid 
    } else {
        const mainParticipant = stanza.attrs.participant
        if (mainParticipant) {
            if (mainParticipant.endsWith('@lid')) {
            
                participant = mainParticipant
                participantPn = stanza.attrs.participant_pn || stanza.attrs.sender_pn
            } else {
                participantPn = mainParticipant
                participant = stanza.attrs.participant_lid || stanza.attrs.sender_lid
            }
        } else {
            participantPn = stanza.attrs.sender_pn || from
            participant = stanza.attrs.sender_lid
        }
    }
    ...
    if (WABinary_1.isJidUser(from)) {
        ...
        if (!participantPn && !participant) {
            throw new boom_1.Boom('No participant in group message')
        }
        msg_type = 'group'
        author = participantPn || participant
        chat_id = from
    } else if (WABinary_1.isJidBroadcast(from)) {
        if (!participantPn && !participant) {
            throw new boom_1.Boom('No participant in broadcast message')
        }
        const isParticipantMe = isMe(participantPn || participant)
    ...
    const fromMe = WABinary_1.isJidNewsletter(from) 
        ...
        : WABinary_1.isLidUser(from) 
            ? isMeLid(participant || stanza.attrs.from) 
            : isMe(participantPn || stanza.attrs.from)
    ...
    const key = {
        ...
        participantPn: participantPn ? WABinary_1.jidNormalizedUser(participantPn) : undefined,
    }
